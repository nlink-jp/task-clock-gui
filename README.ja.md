...
- **その場のアクション**: 即時実行（2 クリックのインターロック付き —
  ...
  reload、最終 run ログの Finder 表示 — 失敗は操作したその画面に表示され、
  次の操作をするかパネルを閉じるまで消えない（裏で走る状態ポーリングが
  操作の返事を消すことはない）。config で `enabled = false` のタスクには
  スイッチを出さない（その層は tasks.d の領分で、何も起きないコントロール
  は嘘になるため）
